...
def main():
    ...
    X_train_seq = np.expand_dims(X_train_transformed, axis=1)
    X_test_seq = np.expand_dims(X_test_transformed, axis=1)
    input_shape = (X_train_seq.shape[1], X_train_seq.shape[2])
    model = build_model(input_shape)
    es = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
    history = model.fit(X_train_seq, y_train, epochs=50, batch_size=128, validation_split=0.2, callbacks=[es], verbose=1)
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    cv_scores = []
    for train_idx, val_idx in skf.split(X_train_seq, y_train):
        model_cv = build_model(input_shape)
        model_cv.fit(X_train_seq[train_idx], y_train.iloc[train_idx], epochs=20, batch_size=128, verbose=0)
        scores = model_cv.evaluate(X_train_seq[val_idx], y_train.iloc[val_idx], verbose=0)
    ...
    performance_csv_path = r'D:\Optimization-Research\Hybrid_Whale_RIME_IDS_Project\UNSW_NB15\src\Basic_Models\Results\LSTM.csv'
    performance_data = {
